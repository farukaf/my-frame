...
public sealed record WorldStateSyncResult(string State, int Records, string? RevisionId, string? ErrorCode);
...
public sealed class WorldStateSyncService(ILogger<WorldStateSyncService> logger)
{
    public async Task<WorldStateSyncResult> RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        var publication = await host.RunWorldStateOnceAsync(new WorldStateClient(client), cancellationToken: cancellationToken);
        var status = await database.GetStatusAsync("worldstate-pc", cancellationToken);
        if (publication is not null)
            return new("published", publication.RecordCount, publication.RevisionId, null);

        logger.LogWarning("World State synchronization failed with {ErrorCode}", status?.ErrorCode);
        return new(status?.LastRunState ?? "failed", 0, status?.ActiveRevisionId, status?.ErrorCode ?? "SYNC_FAILED");
    }
}
