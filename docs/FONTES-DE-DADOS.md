...
| DE3 | [Public Export: índice EN](https://origin.warframe.com/PublicExport/index_en.txt.lzma) e [base de conteúdo](https://content.warframe.com/PublicExport/) | Endpoints candidatos da DE, descobertos por referências comunitárias. Payload e descompressão ainda não executados no projeto. F3 deve validar índice, nomes de arquivos, hashes e idiomas disponíveis. |
| DE4 | [World State](https://content.warframe.com/dynamic/worldState.php) | Endpoint candidato da DE. Validar plataforma, formato de datas, IDs de tabelas e cobertura das bounties; não presumir instruções completas de cada etapa. |
